"""
src/color/color_space.py
Color space transformations (RGB <-> YUV) based on ITU-R BT.601 and BT.709 standards.

RGB represents colors via primary additive channels: Red, Green, Blue.
YUV (YCbCr) separates luminance (Y = achromatic brightness/detail) from
chrominance (U = Cb = blue difference, V = Cr = red difference).

Human Visual System (HVS) Justification:
The human retina contains approximately 120 million rods (sensitive to luminance/intensity)
and only 6 million cones (sensitive to color). HVS is significantly more sensitive to spatial
variations and high frequencies in luminance than in chrominance. Standard lossy codecs
(JPEG, AVC, HEVC) exploit this via 4:2:0 chroma subsampling.
Extracting Y channel features enables deep analysis of brightness, contrast, edges, and textures,
while U and V capture chroma variance and color complexity.
"""
...
def rgb_to_yuv(rgb_img: np.ndarray, standard: str = "BT601") -> np.ndarray:
    """
    Converts an RGB uint8 image [0, 255] to YUV (YCbCr) float32 [0, 255].
...
def get_luminance(rgb_img: np.ndarray, standard: str = "BT601") -> np.ndarray:
    """
    Extracts 2D luminance (Y) map as float32 in range [0, 255].
    """
    if rgb_img.ndim == 2:
